<?php
/** Guarded editable-page update for the repositioned finance graphic. */
...
if ( ! $base || ! is_dir( $base ) ) {
    WP_CLI::error( 'Usage: wp eval-file apply-revision-20260924e.php OUTPUT_DIR [dry-run]' );
}
$manifest_file = is_readable( $base . '/revision-manifest-20260924e.json' )
    ? $base . '/revision-manifest-20260924e.json' : $base . '/manifest.json';
$manifest = is_readable( $manifest_file ) ? json_decode( file_get_contents( $manifest_file ), true ) : null;
$record = $manifest['report']['page'] ?? null;
$file = $base . '/page.html';
$post = get_post( 67 );
if ( ! is_array( $manifest ) || ( $manifest['revision'] ?? '' ) !== '2026-09-24e' ||
...
if ( get_post_type( $id ) !== 'attachment' || ! str_ends_with( (string) get_attached_file( $id ), '/' . $filename ) ) {
    WP_CLI::error( 'Missing media attachment' );
}
WP_CLI::log( 'Guards passed: page 67 and new finance image.' );
if ( $dry_run ) {
...
WP_CLI::success( 'Revision 2026-09-24e installed and verified.' );
