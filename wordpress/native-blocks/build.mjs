...
if (process.argv.includes('--carousel')) {
  ...
  await carousel(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
...
if (process.argv.includes('--revision-20260921')) {
  const index = process.argv.indexOf('--revision-20260921');
  const { revision20260921 } = await import('./revision-20260921.mjs');
  await revision20260921(w, origin, process.argv[index + 1], process.argv[index + 2], process.argv[index + 3]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260924')) {
  const index = process.argv.indexOf('--revision-20260924');
  const { revision20260924 } = await import('./revision-20260924.mjs');
  await revision20260924(w, origin, process.argv[index + 1], process.argv[index + 2], process.argv[index + 3]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260924b')) {
  const index = process.argv.indexOf('--revision-20260924b');
  const { revision20260924b } = await import('./revision-20260924b.mjs');
  await revision20260924b(w, origin, process.argv[index + 1], process.argv[index + 2], process.argv[index + 3]);
  dom.window.close();
  process.exit(0);
}
...
if (process.argv.includes('--revision-20260924d')) {
  const index = process.argv.indexOf('--revision-20260924d');
  const { revision20260924d } = await import('./revision-20260924d.mjs');
  await revision20260924d(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260924g')) {
  const index = process.argv.indexOf('--revision-20260924g');
  const { revision20260924g } = await import('./revision-20260924g.mjs');
  await revision20260924g(w, origin, process.argv[index + 1]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260925')) {
  const index = process.argv.indexOf('--revision-20260925');
  const { revision20260925 } = await import('./revision-20260925.mjs');
  await revision20260925(w, origin, process.argv[index + 1]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260925b')) {
  const index = process.argv.indexOf('--revision-20260925b');
  const { revision20260925b } = await import('./revision-20260925b.mjs');
  await revision20260925b(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
...
if (process.argv.includes('--revision-20260925d')) {
  const index = process.argv.indexOf('--revision-20260925d');
  const { revision20260925d } = await import('./revision-20260925d.mjs');
  await revision20260925d(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260925e')) {
  const index = process.argv.indexOf('--revision-20260925e');
  const { revision20260925e } = await import('./revision-20260925e.mjs');
  await revision20260925e(w, origin, process.argv[index + 1]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260924f')) {
  ...
  await revision20260924f(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
if (process.argv.includes('--revision-20260924e')) {
  const index = process.argv.indexOf('--revision-20260924e');
  const { revision20260924e } = await import('./revision-20260924e.mjs');
  await revision20260924e(w, origin, process.argv[index + 1], process.argv[index + 2]);
  dom.window.close();
  process.exit(0);
}
...
const button = (text, url, attrs = {}) => b('buttons', {layout:{type:'flex',justifyContent:'left'},...attrs}, [b('button', {text, url, className:'is-style-outline', style:{color:{text:blue},border:{color:blue,width:'1px',radius:'5px'},spacing:{padding:{top:'10px',bottom:'10px',left:'20px',right:'20px'}},typography:{fontSize:'14px',fontWeight:'600'}}})]);
const more = (url, attrs = {}) => button('Подробнее', url, attrs);
const heroMore = url => more(url,{style:{spacing:{margin:{top:'clamp(36px, 10vw, 120px)'}}}});
...
const hero = g([tabs('hero-proposals',c.hero.map((x,i)=>({label:['Провайдеры услуг','Финансовые условия','ПО ALLVEND'][i],blocks:[cols([[h(x.title,i===0?1:2,{fontSize:'xx-large'}),...(x.text?[p(x.text)]:[]),heroMore(x.url)],[image(x.image,x.alt)]],['40%','60%'])]})),'Главные предложения SkySend')],{align:'full',style:{color:{background:ice},dimensions:{minHeight:'500px'},spacing:{padding:{top:'8px',bottom:'12px',left:'24px',right:'24px'}}},layout:{type:'constrained',contentSize:'1120px',wideSize:'1120px'},metadata:{name:'Три баннера'}});
const partners = section('participants',[grid(c.partners.map(x=>g([cols([[image(x.image,x.title,{width:'92px',height:'74px',scale:'cover'})],[h(x.title,3,{style:{typography:{fontSize:'17px',fontWeight:'700'}}}),p(x.text,{style:{typography:{fontSize:'14px'}}})]],['28%','72%'],{isStackedOnMobile:false,style:{spacing:{blockGap:{top:'12px',left:'16px'}}}})],{metadata:{name:x.title},style:{spacing:{padding:{top:'14px',bottom:'14px'},blockGap:'8px'}}})),'310px'),grid(c.stats.map(x=>g([p(x.value,{style:{color:{text:blue},typography:{fontSize:'30px',fontWeight:'800'}}}),p(x.label,{style:{typography:{fontSize:'13px'}}})],{style:{spacing:{blockGap:'4px'}}})),'140px',{style:{border:{top:{color:'#dbe5f1',width:'1px'}},spacing:{padding:{top:'20px'},blockGap:'20px'}}})]);
